...
mobBar.addEventListener('click', function (e) {
  if (e.target.tagName === 'A' || e.target.closest('.page-header__close-btn')) {
    if (e.target.closest('.page-header__close-btn')) {
      header.classList.toggle('is-visible');
      addScroll();
    } else {
      setTimeout(function () {
...
if (document.querySelector('.product-desc__title-wrap')) {
  var thumb = document.querySelector('.product-desc__title-wrap'),
      content = document.querySelector('.product-desc__txt');

  var tab = function tab() {
    if (document.documentElement.clientWidth <= 600) {
      thumb.dataset.tab = true;
      thumb.addEventListener('click', toggleTab);
    }
  };

  var toggleTab = function toggleTab() {
    thumb.classList.toggle('is-active');
    content.classList.toggle('visible');
  };

  tab();
  window.addEventListener('resize', function (e) {
    if (!thumb.dataset.tab) {
      tab();
    } else if (thumb.dataset.tab && document.documentElement.clientWidth >= 600) {
      thumb.removeAttribute('data-tab');
      thumb.classList.remove('is-active');
      thumb.removeEventListener('click', toggleTab);
...

{
  var modalBtn = document.querySelectorAll('#modal-btn'),
      _closeBtn = document.querySelectorAll('.modal__close-btn'),
      modalOverlay;
  ...
  modalBtn.forEach(function (openBtn) {
    openBtn.addEventListener('click', function (e) {
      e.preventDefault();
      stopedScroll();
      var overlay = openBtn.dataset.modal;
      modalOverlay = document.getElementById("".concat(overlay));
      modalOverlay.classList.add('open');
    });
  });

  _closeBtn.forEach(function (btn) {
...
},{"imask":1,"lazysizes":2,"lazysizes/plugins/parent-fit/ls.parent-fit":3}]},{},[4]);
